...
def get_stock_bs(dict_filter=None, is_reload=True, is_write=True):
    if not is_reload:
        ...
        for i_symbol, symbol in zip(range(len(symbols)), symbols):
            ...
            for _time, i in zip(_time_list, range(len(pd_price_fine_symbol))):
                ...
                if is_start:
                    ...
                    is_act, is_start, is_first = True, True, False
                    weight = 0
                else:
                    is_start = False
                if is_act:
...
if __name__ == '__main__':
    ...
    n_stock_max = 6
    pd_stock_bs_ori, dict_act_date = get_stock_bs(dict_filter=dict_filter, is_reload=True, is_write=False)
