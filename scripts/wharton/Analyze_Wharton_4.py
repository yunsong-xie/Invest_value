...
if __name__ == '__main__0':
    ...
    keys_front = ['symbol', 'datafqtr', 'marketcap_p', 'marketcap_0', 'marketcap_1', 'marketcap_2', 'marketcap_q1']
...
if 'Training' == 'Training':
    ...
    dict_revenue_growth_min = {'1': 0.0, '0': 0.01}
    dict_book_value_growth_min = {'1': 0.0, '0': 0.01}
    dict_revenue_growth_min = {}
    dict_book_value_growth_min = {}
    dict_revenue_growth_max = {}
    dict_book_value_growth_max = {}
    mc_book_ratio = [1.5, 65]
    mc_revenue_ratio = [1.5, 65]
    ratio_train = 0.4
    ratio_dev = (1 - ratio_train) / 2
    ratio_dev = 0.1
    ...
    # sklearn parameters
    n_regr = 35
    n_estimators_min, n_estimators_max = 200, 350
    ...
    #                        (_pd_data[f'book_value_1'] / _pd_data[f'book_value_2'])]
    _pd_data = _pd_data.loc[((_pd_data.marketcap_p / _pd_data.marketcap_0) <= 10) &
                            ((_pd_data.marketcap_p / _pd_data.marketcap_0) >= 10 ** -1)]
    ...
    _pd_data = _pd_data.loc[((_pd_data.marketcap_p / _pd_data.marketcap_0) <= 6) &
                            ((_pd_data.marketcap_p / _pd_data.marketcap_0) >= 10 ** -1)]
    ...
    def get_summary(dict_pd_data, top_p=0.97):
        dict_sum = {'data_label': [], 'p_value': [], 'mean_all': [], 'mean_top': [], 'std_all': [], 'std_top': [],
                    'num_all': [], 'num_top': [], 'profit_all': [], 'profit_top': [], 'time_min': [], 'time_max': []}
        for label in dict_pd_data:
            ...
            dict_sum['time_min'].append(pd_data.rdq_p.min())
            dict_sum['time_max'].append(pd_data.rdq_p.max())
    ...
    thresholds = [0.1, 0.125, 0.15, 0.175, 0.2, 0.25]
    if 1 == 1:
        val_min_threshold = 0.0
        fig, ax = plt.subplots(2, 4, figsize=(18, 9))
        for i_plot in range(2):
            val_min = val_min_threshold if i_plot == 0 else -5
            pd_data_train_plot = pd_data_train_show.loc[pd_data_train_show[metric_plot] >= val_min]
            pd_data_dev_plot = pd_data_dev_show.loc[pd_data_dev_show[metric_plot] >= val_min]
            pd_data_test_plot = pd_data_test_show.loc[pd_data_test_show[metric_plot] >= val_min]
            pd_data_test_extra_plot = pd_data_test_extra_show.loc[pd_data_test_extra_show[metric_plot] >= val_min]
