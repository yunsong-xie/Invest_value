...
def get_data_from_db():
    ...
    year_desc_positive_list = ['Operating Activities - Net Cash Flow', 'profit', 'Cash and Short-Term Investments', 'Revenue - Total',
                               'Stockholders Equity - Total', 'Current Assets - Total']
    year_desc_grow_dict = {'ALL': {0: ['Operating Activities - Net Cash Flow', 'Revenue - Total',]},
                           0: {
                               0.001: ['Revenue - Total'],
                               # 0.0015: ['Stockholders Equity - Total']
                           },
                           1: {0: ['Revenue - Total', 'Stockholders Equity - Total']}, }
    ...
    pd_data = pd_data.loc[(~pd_data.rdq_pq1.isna()) | (pd_data.rdq_0 >= common_func.date(-120))]

    pd_study = pd_data.loc[pd_data['symbol'] == 'AAPL']
...
if __name__  ==  '__main__0':
    try:
        ...
        pkl_path_holding = max(glob.glob(f'hold_data/pd_hold*.pkl'))
        pd_holding_read = pd.read_pickle(pkl_path_holding)
...
if 'Define Function' == 'Define Function':
        ...
        for i_regr in range(len(pd_estimator_thread)):
            ...
            if dict_transform_thread['regr_type'] == 'xgboost_GB':
                ...
                regr2 = RandomForestRegressor(n_estimators=n_estimators, max_samples=subsample, max_depth=max_depth, random_state=state2,
                                              n_jobs=-1, min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf)
            elif dict_transform_thread['regr_type'] == 'linear':
    ...
    def invest_period_operation(pd_fr_record, pd_holding, pd_data_operate, dict_decision_time, transaction, holding_stocks_start, seed=None):
        ...
        if 'update_holding_metric' == 'update_holding_metric':
            if bool_metric_recalculate & (len(pd_holding) > 1) & (len(regr_list) > 1):
        ...
        if holding_stocks_start is not None:
            pd_data_eval_operation = pd_data_eval_operation.loc[pd_data_eval_operation.symbol.isin(holding_stocks_start)]
        ...
        if 'y_median' not in dict_transform:
            dict_transform['y_median'] = -100
            dict_transform['y_std'] = 0
            dict_transform['features_x'] = []
            dict_transform['features_x_select'] = []
        ...
        keys_exclude = ['mean', 'std', 'dict_transform', 'y_std', 'regr_list']
...
if __name__ == '__main__':
    ...
    dict_revenue_growth_min_soft, dict_book_value_growth_min_soft = {'1': -0.1, '0': 0.3}, {'1': -0.1, '0': 0.3}
    dict_revenue_growth_min, dict_book_value_growth_min = {'1': -0.2, '0': 0.2}, {'1': -0.2, '0': 0.2}
    ...
    n_estimators_range, max_depth_range, subsample_range = [150, 150], [6, 6], [0.85, 0.85]
    min_samples_split, min_samples_leaf = 2, 1
    training_num_p_min = 0.75
    _decision_time_start = '2023-01-01'
    bool_pseudo = True
    bool_read_hold = False
    n_trials = 1
    n_regr = 10
    aug_size_train, aug_sigma_train = 15, 0.1
    aug_size_pseudo, aug_sigma_pseudo = 5, 0.1
    major_feature = 'book_value'

    # holding_stocks_start = ['HOLX', 'XPEL']
    holding_stocks_start = None
    ...
    if bool_read_hold:
        _decision_time_start = pd_holding_read.iloc[0].decision_time_end
        _decision_time_start = str(pd.to_datetime(_decision_time_start) + pd.to_timedelta('1 day'))[:10]
    month_start, now = date_month_convertion(_decision_time_start), str(datetime.datetime.now())[:10]
    _decision_time_end = date_month_convertion(int(np.ceil((date_month_convertion(now) - month_start + 1) / 3)) * 3 + month_start)
    _decision_time_end = str(pd.to_datetime(_decision_time_end) - pd.to_timedelta('1 day'))[:10]
    print(f'Start time: {_decision_time_start}\nEnd time: {_decision_time_end}')
    ...
    for i in dict_revenue_growth_min:
        _pd_data[f'revenue_{i}_growth'] = _pd_data[f'revenue_{i}'] / _pd_data[f'revenue_{int(i) + 1}'] - 1
        _pd_data[f'book_value_{i}_growth'] = _pd_data[f'book_value_{i}'] / _pd_data[f'book_value_{int(i) + 1}'] - 1
    ...
    # pd_base = pd_base.loc[(_pd_data[f'revenue_q0'] / pd_base[f'revenue_q4']) >= (pd_base[f'revenue_1'] / pd_base[f'revenue_2'])]
    # pd_base = pd_base.loc[(pd_base[f'book_value_q0'] / pd_base[f'book_value_q4']) >=
    #                         (pd_base[f'book_value_1'] / pd_base[f'book_value_2'])]
    ...
    for i_trial in range(n_trials):
        ...
        if bool_read_hold:
            pd_holding = pd_holding_read.copy()
        ...
        for i_period in range(n_period + 1):
            ...
            if i_period == 0:
                _holding_stocks_start = None if bool_read_hold else holding_stocks_start
                pd_fr_record, pd_holding, dict_transform_save = invest_period_operation(pd_fr_record, pd_holding, pd_data_operate,
                                                                                        dict_decision_time, transaction, _holding_stocks_start)
            else:
                pd_fr_record, pd_holding, dict_transform_save = invest_period_operation(pd_fr_record, pd_holding, pd_data_operate,
                                                                                        dict_decision_time, transaction, None)
