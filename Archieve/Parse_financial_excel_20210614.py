...
class ParseExcel:
    ...
    def main_parse(self):
        dir_financial_report = max(glob.glob(f'{DIR}/static/Financial_reports/*'))
        ...
        for xls_file in xls_files:
            ...
            # Get the price info 35 days after the earning date,
            # The date parsed here is the fiscal date, not the actual filling date. Only after round 35 days, the report is available
            # TODO: get the Accurate original filing date
            time_list_price = [common_func.unix2date(common_func.date2unix(i) + 3600 * 24 * 35)[:10] for i in time_list]
            ...
            try:
                ...
                pd_price = stock_price.get_price_dates(symbol, dates=time_list_price)
                pd_price['time_request'] = pd_price['time_request'].replace(dict_time_replace)
            dict_time_replace_to_trading_day = {i[0]: i[1] for i in pd_price[['time_request', 'time']].values}
            pd_price = pd_price.rename(columns={'close': 'Price'})
            pd_sum_symbol['time'] = pd_sum_symbol['time'].replace(dict_time_replace_to_trading_day)
            ...
            pd_market_cap_entry = pd_market_cap_entry.loc[pd_market_cap_entry.value != 'NA'].drop_duplicates().copy()
...
def get_pd_growth(pd_sum_select, pd_mc_asset, dict_item_id, n_year=2, mp_id=0, mp_queue=None):
...
if __name__ == '__main__1':
    ...
    item_id_list = list(dict_item_id.values())
    item_id_include_list = item_id_list + ['Tangible Book Value', 'Market Cap']
    pd_sum = pd_sum.loc[(pd_sum.item_id.isin(item_id_include_list)) & (pd_sum.tq > 2003)]
    pd_mc_asset, ratio_col_name = stock_an.get_stock_ratio(pd_sum, item_id_x='Tangible Book Value',
                                                           item_id_y='Market Cap', col_val='value')
    pd_mc_asset = pd_mc_asset.rename(columns={ratio_col_name: 'PB', 'time': 'year'})
    pd_mc_asset = pd_mc_asset[['exchange_ticker', 'year', 'PB']]
    pd_mc_asset = pd_mc_asset.loc[(pd_mc_asset['PB'] < 10000)]
    ...
    pd_mc_asset, ratio_col_name = stock_an.get_stock_ratio(pd_sum_select, item_id_x='Tangible Book Value',
                                                           item_id_y='Market Cap', col_val='value')
    pd_mc_asset = pd_mc_asset.rename(columns={ratio_col_name: 'PB', 'time': 'year'})
    pd_mc_asset = pd_mc_asset[['exchange_ticker', 'year', 'PB']]
    ...
    for i in range(n_process):
        ...
        mp.Process(target=get_pd_growth, args=(pd_sum_sec, pd_mc_asset, dict_item_id, n_year, i, mp_queue)).start()

    while len(dict_progress['result']) != n_process:
        time.sleep(0.25)
        dict_progress = mp_print_progress(mp_queue, time_start, dict_progress)

    pd_data_growth = pd.concat(dict_progress['result'])

    pd_data_growth.to_pickle(f'{DIR}/static/pd_data_grow_{common_func.date(0)}.pkl')
    pd_item_id.to_pickle(f'{DIR}/static/pd_data_grow_item_id_{common_func.date(0)}.pkl')
...
if __name__ == '__main__':
    ...
    pd_data_plot_1 = pd_data_plot_0.loc[(pd_data_plot_0['growth_C'] > 0.3) & (pd_data_plot_0['growth-1_C'] > 0.3)].copy()
    ...
    pd_data_plot_1 = pd_data_plot_1.loc[(pd_data_plot_1['PB'] < 10) & (pd_data_plot_1['PB'] > 2)]
    ...
    for i_symbol, symbol in zip(range(len(symbols)), symbols):
        ...
        if len(pd_symbol) > 1:
            ...
            bins = np.asarray(
                [pd_symbol.time.min()] + list(pd_selling.time + pd.to_timedelta('0.5 days')) + [pd_symbol.time.max()]).astype(
                np.datetime64)
        ...
        if ((pd_symbol.iloc[-1].time < pd.to_datetime(common_func.date(-hold_day_min))) & (len(pd_selling) < len(pd_buying))):
            ...
            pd_stock_act_list.append(pd_selling_final)


        pd_stock_act_list.append(pd_selling)
        pd_stock_act_list.append(pd_buying)

    pd_stock_bu_se = pd.concat(pd_stock_act_list).sort_values(by=['time', 'symbol'])

    pd_stock_final, pd_reward = bu_se_2_profit(pd_stock_bu_se)
