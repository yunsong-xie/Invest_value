...
class StockAnalyze:
    ...
    def get_stock_bs(self, dict_filter=None, is_reload=True, is_write=True):
        if not is_reload:
            dir_data = f'{DIR}/static/analysis'
            path_pd_stock_bs = max(glob.glob(f'{dir_data}/pd_stock_bs_*.pkl'))
            path_dict_act_date = max(glob.glob(f'{dir_data}/dict_act_date_*.pkl'))

            pd_stock_bs = pd.read_pickle(path_pd_stock_bs)
            with open(path_dict_act_date, 'rb') as handle:
                dict_act_date = pickle.load(handle)

        else:
            path_growth = max(glob.glob(f'{DIR}/static/analysis/pd_data_grow_2*.pkl'))
            ...
            pd_growth_1 = pd_growth_1.loc[(pd_growth_1['growth_A'] >= 0.0) & (pd_growth_1['growth-1_A'] >= 0.0)]
            pd_growth_1 = pd_growth_1.loc[(pd_growth_1['growth_B'] >= 0.0) & (pd_growth_1['growth-1_B'] >= 0.0)]
            pd_growth_1 = pd_growth_1.loc[(pd_growth_1['growth_C'] >= 0.0) & (pd_growth_1['growth-1_C'] >= 0.0)]
            pd_growth_1 = pd_growth_1.loc[(pd_growth_1['growth_D'] >= 0.0) & (pd_growth_1['growth-1_D'] >= 0.0)]
            pd_growth_1 = pd_growth_1.loc[(pd_growth_1['growth_E'] >= 0.0) & (pd_growth_1['growth-1_E'] >= 0.0)]
            ...
            for i_symbol, symbol in zip(range(len(symbols)), symbols):
                ...
                pd_price = pd_price.merge(pd_symbol[['time', 'growth_A', 'growth-1_A', 'growth_C', 'growth-1_C', 'PB', 'MC_TCA',
                ...
                is_start, weight_final, time_sec = True, -1, ''
            ...
            pd_stock_bs.loc[pd_stock_bs.PB <= 0, 'weight_final'] = 0
            pd_stock_bs = pd_stock_bs.sort_values(by=['time', 'symbol'])

            date_now = common_func.date(0)
            path_pd_stock_bs = f'{DIR}/static/analysis/pd_stock_bs_{date_now}.pkl'
            path_dict_act_date = f'{DIR}/static/analysis/dict_act_date_{date_now}.pkl'

            if is_write:
                pd_stock_bs.to_pickle(path_pd_stock_bs)
                with open(path_dict_act_date, 'wb') as handle:
                    pickle.dump(dict_act_date, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def bu_se_2_profit_fine(dict_pd_stock_bs, n_stock_max=5, is_update_weight=False):
    ...
    for symbol in dict_pd_stock_bs:
        ...
        pd_stock_bs_symbol = dict_pd_stock_bs[symbol].copy()
        _growth, _PB = pd_stock_bs_symbol.growth_C, pd_stock_bs_symbol.PB
        pd_stock_bs_symbol['weight'] = get_trading_weight(_growth, _PB)
        if is_update_weight:
            pd_stock_bs_symbol['weight_final'] = pd_stock_bs_symbol['weight']
        else:
            time_financial_list = pd_stock_bs_symbol.time_financial.unique()
            for time_financial in time_financial_list:
                _weight = pd_stock_bs_symbol.loc[pd_stock_bs_symbol['time'] >= time_financial].iloc[0].weight
                pd_stock_bs_symbol.loc[pd_stock_bs_symbol['time_financial'] == time_financial, 'weight_final'] = _weight
        pd_stock_bs_symbol.loc[pd_stock_bs_symbol['sec'] == -1, 'weight_final'] = 0
        pd_stock_bs_recal_list.append(pd_stock_bs_symbol)
        pd_stock_bs_ori_list.append(pd_stock_bs_symbol_ori)

    pd_stock_bs_recal = pd.concat(pd_stock_bs_recal_list)
    ...
    pd_stock_act_sim = pd_stock_act_sim[['time', 'symbol', 'Price', 'weight_final']]
...
def get_trading_weight(growth, pb, min_growth=0.3, max_growth=10, growth_slope=100, max_pb_1=25, max_pb_2=100):
...
def plot_score(min_growth=0.3, max_growth=5, growth_slope=10, max_pb_1=25, max_pb_2=100):
    from matplotlib import cm

    growth, pb = np.meshgrid(np.linspace(min_growth, max_growth), np.linspace(2, max_pb_2))
    score = get_trading_weight(growth, pb, min_growth, max_growth, growth_slope, max_pb_1, max_pb_2)

    fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    surf = ax.plot_surface(growth, pb, score, cmap=cm.coolwarm, linewidth=0, antialiased=False)

# plot_score(min_growth=0.3, max_growth=5, growth_slope=20, max_pb_1=25, max_pb_2=100)


if __name__ == '__main__':

    dict_filter = {'>': [['growth_C', 0.5], ['growth-1_C', 0.2], ['MC_TCA', 5], ['growth_A', 'growth-1_A'],
                         ],
                   '<': [['MC_TCA', 35], ['PB', 35]]}

    n_stock_max = 6
    pd_stock_bs, dict_act_date = stock_an.get_stock_bs(dict_filter=dict_filter, is_reload=True, is_write=False)
    pd_stock_bs['time'] = pd_stock_bs['time'].astype(str).str[:10]
    pd_stock_bs['time_financial'] = pd_stock_bs['time_financial'].astype(str).str[:10]
    time_act_list = list(pd_stock_bs.loc[pd_stock_bs.act].time)
    ...
    pd_reward = bu_se_2_profit_fine(dict_pd_stock_bs=dict_pd_stock_bs, n_stock_max=n_stock_max,
    ...
    pd_recommend = pd_recommend.merge(pd_price_info, on='symbol', how='inner')[['symbol', 'Price', 'num']]
